...
// Reflecting on this: at what point is this just a weird string?
class StaticString {
public:
    explicit StaticString(size_t buffer_length) : _buffer(buffer_length, '\0') {
        _buffer.resize(capacity()); // this is to make
    }
...
public:
    ...
    bool starts_with(std::string_view needle) { return _buffer.compare(0, needle.size(), needle, 0) == 0; }
    bool ends_with(std::string_view needle) {
