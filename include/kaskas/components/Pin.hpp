...
public:
    ...
    DigitalInputPin(uint8_t pin, bool pull_up = false) : Pin(pin), pull_up(pull_up) {}
    DigitalInputPin(Config& cfg) : Pin(cfg.pin), pull_up(cfg.pull_up) {}
...
public:
    ...
    DigitalOutputPin(uint8_t pin, bool active_on_low = false) : Pin(pin), _active_on_low(active_on_low) {}
    ...
    void set_state(DigitalState state) {
        if (state == true)
            digitalWrite(pin, !_active_on_low);
        else
            digitalWrite(pin, _active_on_low);
        _active = state;
    }
